    ...
    const fetchUserProfile = async () => {
      try {
        ...
        const response = await axios.get(
          `${server_url}user/profile/${username}`,
          {
            headers: {
  ...
  return (
    ...
    <>
      <div className="flex flex-col items-center mt-20">
        <div className=" bg-[#9cc034] flex flex-col justify-center items-center gap-2 text-white rounded-[2rem]  md:p-20 p-5 pb-10">
          <div className="md:w-1/2">
            <img className="" src={bgImage} alt="image" />
          </div>
          <div className="flex flex-row">
            <Link
              to="/user/updateprofile"
              className="bg-brand flex justify-center items-center rounded-lg font-bold p-1 mt-2"
            >
              Update Profile
            </Link>

            <img
              className="h-[10rem] w-[10rem] rounded-full mt-[-7rem] border-4 border-gray-800 "
              src="https://img.freepik.com/premium-photo/anime-male-avatar_950633-914.jpg"
              alt="mamun"
            />

            <Link
              to="/user/passwordchange"
              className="bg-brand rounded-lg flex justify-center items-center p-1 font-bold mt-2"
            >
              Change Password
            </Link>
          </div>
          <div className="flex flex-col text-xl md:text-3xl self-start text-black my-2">
            <h1>
