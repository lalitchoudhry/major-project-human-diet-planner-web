...
const DietCalendar = () => {
  ...
  useEffect(() => {
    const fetchDietDates = async () => {
      try {
        ...
        const response = await axios.get(`${server_url}diet/diet-dates/${username}`);
  ...
  const addDietDate = async () => {
    if (selectedDate && !isDateDietMaintained(selectedDate.toDateString())) {
      try {
        ...
        const server_url = process.env.REACT_APP_SERVER_URL;
        const response = await axios.post(`${server_url}diet/track/${username}`, {
  ...
  return (
    <div className="flex-col mt-20 overflow-hidden md:p-20 p-5 flex items-center justify-center">
      <h3 className="text-2xl md:text-3xl font-bold mb-10" style={{ fontFamily: 'Roboto, sans-serif' }}>
      ...
      <div className="flex flex-col-reverse md:flex-row items-center justify-between w-full h-full">
        <div className="flex-grow">
          <div className="mb-4">
            <button onClick={addDietDate} className="bg-green-500 text-white py-2 rounded">
        ...
        <div className="md:w-2/4 md:pl-1">
