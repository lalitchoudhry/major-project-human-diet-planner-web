...
const WorkReco = () => {
    ...
    const fetchUserProfile = async () => {
        try {
          const server_url = process.env.REACT_APP_SERVER_URL;
            ...
            const response = await axios.get(`${server_url}user/profile/${username}`, {
                headers: {
  ...
  return (
<div className="flex justify-center min-h-screen bg-gradient-to-r from-beige-100 to-beige-300 bg-secondary md:p-20 p-5">
  <div className=" w-full mx-auto mt-20">
    {/* Form Section */}
    <div className=" flex flex-col">
      <input
        className="mb-2 p-2 border rounded-md w-full md:w-1/3"
        type="text"
        value={requiredCalories}
        onChange={(e) => setRequiredCalories(e.target.value)}
        placeholder="Plaese Enter Calories To Burn ..."
        required
      />
      <button
        className="mb-4 px-4 py-2 bg-brand text-white rounded-md shadow-md w-full md:w-1/3"
        onClick={getMealPlan}
      >
    ...
    <div className="md:p-4">
        <div className="border border-gray-300 rounded-md p-4 text-xs md:text-base" dangerouslySetInnerHTML={{ __html: mealPlan }} />
      </div>
    </div>
  </div>
</div>
